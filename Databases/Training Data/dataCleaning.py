...
def getTrainingData():
    ...
    source = os.path.dirname(Path(__file__).parent.parent)
    fileName = os.path.join(source, 'Classifier', 'CleanDB.csv')
    cleanDF[['Product', 'review', 'rating', 'type']].to_csv(fileName, index = False)
    return
